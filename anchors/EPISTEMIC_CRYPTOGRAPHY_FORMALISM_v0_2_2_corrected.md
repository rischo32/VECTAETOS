...
**Version:** v0.2.2 — LaTeX and equation normalization patch  
**Status:** CANONICAL WORKING PATCH / RENDER-SAFE EQUATION PATCH  
...
=
\mathsf{SHA3\text{-}512}(\mathrm{canon}(\ell_t))
```
...
```math
M_i = \mathsf{Hash}(\mathrm{canon}(\ell_{t_i}))
```
...
arbitrary Delta is accepted as representable
Delta is used as decision signal
d2Delta violation is treated as normal projection
```
