...
class BlobIntegrityError(RuntimeError):
    ...
    This exception is for guard infrastructure flow only. It is not ontology.
    """
...
def record_from_mapping(raw: Mapping[str, Any], *, index: int) -> BlobRecord:
    ...
    for key in required:
        if key not in raw:
            raise BlobIntegrityError(f"manifest blob #{index} missing required field: {key}")
    ...
    try:
        ...
        raise BlobIntegrityError(f"manifest blob #{index} has invalid size: {path}") from exc
    ...
    if sha3_512_value is not None:
        ...
        if len(sha3_512) != 128 or any(char not in "0123456789abcdef" for char in sha3_512):
            raise BlobIntegrityError(f"manifest blob #{index} has invalid sha3_512: {path}")
...
def load_manifest(manifest_path: Path | str) -> BlobManifest:
    ...
    try:
        ...
        raise BlobIntegrityError(f"manifest invalid JSON: {normalize_repo_path(source)}") from exc
    except OSError as exc:
        raise BlobIntegrityError(f"manifest unreadable: {normalize_repo_path(source)}") from exc
    ...
    if schema_version != SUPPORTED_SCHEMA_VERSION:
        raise BlobIntegrityError(
            f"manifest schema_version must be {SUPPORTED_SCHEMA_VERSION!r}, got {schema_version!r}"
        )
    ...
    if hash_algorithm != DEFAULT_HASH_ALGORITHM:
        raise BlobIntegrityError(
            f"unsupported hash_algorithm {hash_algorithm!r}; supported: {DEFAULT_HASH_ALGORITHM!r}"
        )
    ...
    if not target.exists():
        return [
            blob_finding(
                ...
                contract_ref=contract_ref,
                protected_object=record.role,
                safer_form="Restore the tracked file or update the manifest through reviewed human process.",
            )
        ]
    ...
    if actual.size != record.size:
        findings.append(
            blob_finding(
                ...
                contract_ref=contract_ref,
                protected_object=record.role,
                observed_pattern=f"expected_size={record.size}; actual_size={actual.size}",
                safer_form="Review the file bytes and update manifest only through explicit human review.",
            )
        )
    ...
    if actual.sha256 != record.sha256:
        findings.append(
            blob_finding(
                ...
                contract_ref=contract_ref,
                protected_object=record.role,
                observed_pattern=f"expected_sha256={record.sha256}; actual_sha256={actual.sha256}",
                safer_form="Treat as repository-state integrity drift; do not infer semantic truth or falsehood from the hash.",
            )
        )
    ...
    if record.sha3_512 is not None and actual.sha3_512 != record.sha3_512:
        findings.append(
            blob_finding(
                ...
                contract_ref=contract_ref,
                protected_object=record.role,
                observed_pattern=f"expected_sha3_512={record.sha3_512}; actual_sha3_512={actual.sha3_512}",
                safer_form="Treat as repository-state integrity drift; do not infer semantic truth or falsehood from the hash.",
            )
        )
    ...
    try:
        ...
        return [
            blob_finding(
                ...
                contract_schema_version=contract_schema_version,
                contract_ref=contract_ref,
                safer_form="Restore or regenerate manifest through explicit human-reviewed process.",
            )
        ]
...
def manifest_record_for_file(
    *,
    ...
    This helper does not write a manifest. The caller must decide whether to
    ...
    if not target.exists() or not target.is_file():
        raise BlobIntegrityError(f"cannot create manifest record for missing/non-file path: {repo_path}")
...
def render_manifest_json(manifest: BlobManifest) -> str:
    return json.dumps(manifest_to_dict(manifest), ensure_ascii=False, indent=2, sort_keys=True) + "\n"
