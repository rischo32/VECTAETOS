...
def _prepend_import_root(root: Path) -> None:
    ...
    cannot appear after an untrusted working-tree path.
    """
...
def _bootstrap_repo_import_path() -> Path:
    ...
    CI sets VECTAETOS_TRUSTED_RUNTIME_ROOT to the base-ref copy under
    `.trusted/base`. When this variable exists, the guard must not prefer the
    PR working tree for `guards.core.*` imports.

    Local direct execution remains supported by falling back to nearby repo
    roots only when no trusted runtime root is declared.
    """
    ...
    if trusted_root_raw:
        ...
        # Fail later with explicit import diagnostics. Do not fall back to the
        # PR working tree while trusted mode was requested.
        _prepend_import_root(trusted_root)
        return trusted_root
    ...
    # Fail later with normal import diagnostics if no repo root is visible.
    return cwd
...
VERSION = "0.2.3-trusted-runtime-root"
