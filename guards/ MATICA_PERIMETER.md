...
Main corrections preserved from v0.4:
...
Current-state corrections in v0.5:

```text
GUARD-12 coherence_vocabulary_guard.py is now ACTIVE,REFAC,MVP
GUARD-31 license_stack_guard.py added as active license-stack perimeter
contracts/perimeter_manifest.json is the current manifest path
contracts/LICENSE_REGISTRY.json is the current license registry path
GUARD_TABLE.md lives in repository root as human-readable guard role map
guards/run_perimeter.py is not active and must not be listed as committed runner
license-stack workflow is report-first and does not use artifact upload action
```
...
REPORT       intentionally report-first before strict enforcement
STAGED       present but not yet promoted to required strict check
```
...
| GUARD-01 | `canonical_ontology_guard.py` | 0 | V0,V1,V7,V14,V15 | E1 | strict | anchors / trusted runtime | ACTIVE,REFAC,MVP | canonical boundary |
...
| GUARD-12 | `coherence_vocabulary_guard.py` | 2 | V3,V8,V9,V12 | E0,E1 | report → strict | vocabulary | ACTIVE,REFAC,MVP,REPORT | coherence terms |
...
| GUARD-31 | `license_stack_guard.py` | 5 | V4,V7,V14,V16 | E1 | report → strict | license / DOI / hash boundary | ACTIVE,MVP,REPORT | license stack integrity |
...
Do not implement all 32 guards before shared core stabilizes.
...
Current standalone guard note:

```text
GUARD-31 license_stack_guard.py currently uses standard-library local checks.
It may later adopt shared findings/reporting core, but must remain read-only.
```
...
8. GUARD-31 license_stack_guard.py
9. GUARD-20 release_claim_guard.py
10. GUARD-19 repo_path_guard.py
11. GUARD-22 contract_traceability_guard.py
12. GUARD-24 anchor_blob_integrity_guard.py
```
...
Not active / not committed as current runner:

```text
guards/run_perimeter.py
```

Current license stack posture:

```text
registry path: contracts/LICENSE_REGISTRY.json
hash manifest path: LICENSES/LICENSE_INTEGRITY_MANIFEST.sha256
workflow mode: report-first
artifact upload action: not used
external checkout action: not used
```
...
GUARD_TABLE.md
    root-level human-readable guard role map

contracts/perimeter_manifest.json
    machine-readable perimeter command / guard registry

contracts/LICENSE_REGISTRY.json
    machine-readable license-stack boundary registry
