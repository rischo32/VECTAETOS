...
Runtime posture:
    This file may be copied to /tmp by the workflow before execution.
    Therefore it bootstraps the repository root into sys.path before importing
    guards.core modules.
...
def _bootstrap_repo_import_path() -> Path:
    """
    Ensure repo root is importable even when this guard is executed from /tmp.

    GitHub workflow runs this guard as /tmp/canonical_ontology_guard.py while
    keeping the repository as current working directory. Python then places
    /tmp on sys.path, not necessarily the repository root.
    """
...
VERSION = "0.2.2-tmp-bootstrap-byte-trace-safe"
...
def detect_byte_trace_drift(changes: Iterable[ChangedPath], head: str) -> list[Finding]:
    ...
    for path in unique_paths:
        ...
        for line_number, line in enumerate(content.splitlines(), start=1):
            ...
            marker_text, marker_index = marker
            excerpt = line.strip()
            if len(excerpt) > 240:
                excerpt = excerpt[:239] + "..."
