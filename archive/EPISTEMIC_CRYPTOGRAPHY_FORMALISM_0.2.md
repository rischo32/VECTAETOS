...
**Version:** v0.2.1  
**Status:** CANONICAL WORKING PATCH  
...
```math
\widehat{R}_{EK} \in \mathfrak{so}(8)
```
...
\widehat{R}_{EK}, & \text{if reconstructed from outputs}
\end{cases}
```
...
```math
\Delta^{EK} \in \operatorname{Im}(d_1)
```
...
h_{\mathrm{topo}}^{EK}
=
\frac{\mu_{\mathrm{tot}}^{EK}}
{\mu_{\mathrm{tot}}^{EK}+A_{\mathrm{tot}}^{EK}+\varepsilon}
```
...
F_{256}(t)
=
\mathrm{SHA256}(\mathrm{canon}(\ell_t))
```
...
F_{3\text{-}512}(t)
=
\mathrm{SHA3\text{-}512}(\mathrm{canon}(\ell_t))
```
...
```math
\mathrm{Vortex} \cap \Psi_{EK} = \varnothing
```
...
## 22. Migration Note from v0.2 to v0.2.1
