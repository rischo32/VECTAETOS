...
```mermaid
flowchart TD
    XI["ZMYSEL / Ξ<br/>epistemic carrier substrate<br/>not operator, not runtime"]
    PHI["Φ = (Σ, R)<br/>invariant singularities + antisymmetric relations"]
    RMK["RMK<br/>relational carrier of R<br/>descriptive continuity only"]
    D["Δ / 𝒟 / E / κ / QE<br/>representability boundary<br/>κ is not threshold"]
    V["Simulation Vortex<br/>trajectory exposure<br/>no objective / no attractor"]
    P["Projection Deformation<br/>descriptive projection only"]
    EK["Epistemic Cryptography<br/>structural audit / provenance trace"]
    TG["TetraGlyph<br/>glyph trace / artifact"]
    LLM["LLM Adapter<br/>language rendering only"]
    HUMAN["Human Interpretation<br/>downstream reading"]

    XI --> PHI --> RMK --> D --> V --> P --> EK --> TG --> LLM --> HUMAN

    NIR["NIR: Non-Intervention Regime<br/>no agency / no optimization / no feedback / no reward"]
    NIR -. applies across .- XI
    NIR -. applies across .- PHI
    NIR -. applies across .- D
    NIR -. applies across .- V
    NIR -. applies across .- EK
    NIR -. applies across .- LLM
```
